...
#include"Socket.h"
 ...
 {
    //初始化了epoll对象的指针，绑定的socket，是否加入到红黑树的判断。
 }
...
void Channel::handleevent()                  //事件处理函数epoll_wait返回的时候调用
{
    if (revents_ & EPOLLRDHUP)               // 对方已关闭，有些系统检测不到，可以使用EPOLLIN，recv()返回0
    {
      closecallback_();  //回调Connection::closecallback
    }
    else if (events_ & (EPOLLIN|EPOLLPRI))    // 接收缓冲区中有数据可以读。
    {
      readcallback_();                        //回调Accept类的newconnection()
    } 
...
void Channel::setwritecallback(std::function<void()>fn)     //设置写事件的回调函数。
{
  writecallback_=fn;
}
