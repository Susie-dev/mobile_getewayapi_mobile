...
private:
    ...
    std::atomic_bool disconnect_;             //客户端是否断开，如果断开设置为true
    ...
    Timestamp lastatime_;                      // 时间戳，创建 Connection 对象时为当前时间，每接收到一个报文，把时间戳更新为当前时间。
...
public:
    ...
    EventLoop* getloop() const { return loop_; } // 返回事件循环

    void onmessage();               //处理对端发过来的信息
