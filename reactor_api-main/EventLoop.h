...
public:
    EventLoop(bool mainloop,int timetvl_=30,int timeout_=80);                                          // 在构造函数中创建Epoll对象ep_。
    ...
    void setepolltimeoutcallback(std::function<void(EventLoop*)> fn); //设置epoll wait()超时的回调函数

    bool isinloopthread();           //判断线程是否为事件循环线程
    ...
    void settimercallback(std::function<void(int)> fn);//将被设置为TcpServer::removSanh
};